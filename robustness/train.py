    ...
    for i, (inp, target) in iterator:
       # measure data loading time
        ...
        output, final_inp = model(inp, target=target, make_adv=adv,
                                  **attack_kwargs)
        loss = train_criterion(model, inp, target) if has_custom_train_loss else train_criterion(output, target)
        # print("loss:", loss)
...
def chance_loss(X, label, model, T=0, eps=0.01, sample_size=10000):
    ...
    model.eval()
    all_im = X.repeat(sample_size, 1, 1, 1)
    all_label = label.repeat(sample_size) 
    print(all_im.size(), all_label.size())
    _, im_spat =model(all_im, all_label, make_adv=True, **kwargs)
    ...
    inf_n = ch.norm(output_ori-output, p=float('inf'), dim=1)
    g = inf_n
    T = d
    ...
    # print(output)
    def min_obj(alpha):
        alpha = Variable(ch.tensor(alpha, requires_grad = True)).cuda()
